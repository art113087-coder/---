    ...
    <header className="sticky top-0 z-40 w-full bg-white/98 backdrop-blur-md border-b border-stone-100">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-stone-50 border border-stone-100 rounded-full p-0.5">
            {LANGUAGES.map(l => (
              <button
                key={l.code}
                onClick={() => setLang(l.code)}
                className={`w-7 h-7 rounded-full flex items-center justify-center text-[8px] font-bold transition-all ${lang === l.code ? 'bg-white shadow-sm text-amber-600' : 'text-stone-400 hover:text-stone-600'}`}
                title={l.label}
              >
                {l.code.toUpperCase()}
              </button>
          ...
          <div className="h-6 w-[1px] bg-stone-100 mx-1 hidden md:block"></div>
